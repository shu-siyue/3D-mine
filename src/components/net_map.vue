...
export default {
  ...
  mounted() {
    ...
    Cesium.Ion.defaultAccessToken =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyYTYyMmJjZi0xZjM2LTQ3NGEtYmExNC03YTYxOGE1YmYyOGYiLCJpZCI6MzAzNDUsInNjb3BlcyI6WyJhc3IiLCJnYyJdLCJpYXQiOjE1OTM1NzU2OTd9.UZ4GQq2vvQflwMn_a8rYtI7mn0JU-ZADiKdSYPqlE4c";
    ...
    var scene = viewer.scene; // 相机是viewer.scene中的属性，用来控制当前可见的域。
    ...
    globe.translucency.frontFaceAlphaByDistance = new Cesium.NearFarScalar(
      ...
      800.0,
      1.0
    ); // 当相机距离表面400米时，将正面透明度设置为0;当相机距离接近800米时，设置为1.0。
    ...
    var url =
      "../Cesium-1.71/Apps/SampleData/models/ParcLeadMine/ParcLeadMine.glb";
    ...
    Cesium.knockout.track(viewModel); // 监测viewModel中的属性
    var toolbar = document.getElementById("toolbar");
    Cesium.knockout.applyBindings(viewModel, toolbar); // 将viewModel对象与html控件绑定
    ...
    function update() {
      ...
      var alpha = Number(viewModel.alpha); // Number() 函数把对象的值转换为数字。
      alpha = !isNaN(alpha) ? alpha : 1.0; // 如果alpha is not a number，就是1.0
      alpha = Cesium.Math.clamp(alpha, 0.0, 1.0); // 第一个参数为要约束的值
    ...
    viewer._cesiumWidget._creditContainer.style.display = "none"; // 去除 版权图标
  }
};
</script>
